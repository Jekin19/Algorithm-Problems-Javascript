...
let clone_iterative = function(root) {
  if (!root) {
    return null;
  }
  ...
  let stack = [root];
  let map = new Map();
  let newRoot = new Node_clone(root.data);
  map.set(root, newRoot);
  while (stack.length > 0) {
    const current = stack.pop();
    let currentNewNode = map.get(current);
    for (let node of current.neighbors) {
      let newNode = map.get(node);
      if (!newNode) {
        newNode = new Node_clone(node.data);
        stack.push(node);
        map.set(node, newNode);
      }
      currentNewNode.neighbors.push(newNode);
    }
  }
  return newRoot;
};
...
let clone = function(root) {
  let nodes_completed = {};
  return clone_iterative(root, nodes_completed);
};
